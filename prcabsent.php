...
if($method=='add_absent'){
    
    $empno=$_POST['empno'];
    $regdate=date("Y-m-d H:i:s");
    $beginabsdate=insert_date($_POST['beginabsdate']);
    $endabsdate=insert_date($_POST['endabsdate']);
...
if (empty($sql)) {
    echo "<p>";
        ...
        echo "<a href='add_absent.php?id=$empno' >กลับ</a>";
        exit();
}

}elseif($_POST['method']=='edit_absent'){
...
$sql=$db->prepare('update absent set beginabsdate=? , endabsdate=? , note=? , regdate=? , recorder=?
        where empno=? and abs_id=?');
$sql->bind_param('ssssiii',$beginabsdate,$endabsdate, $note,$regdate,$recorder,$empno,$abs_id);
$sql->execute();    
