    ...
    <div class="col-lg-12">
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                ...
                //if (!empty($_REQUEST['method']) and !empty($_REQUEST['dep'])) {
...
// สร้างฟังก์ชั่น สำหรับแสดงการแบ่งหน้า   
                    function page_navigator($before_p, $plus_p, $total, $total_p, $chk_page) {
                        global $e_page;
                        global $querystr;
                        $depno = isset($_REQUEST['dep'])?$_REQUEST['dep']:'';
...
ORDER BY wh.his_id desc";
                        if ($_SESSION['Status'] != 'ADMIN') {
                            if ($_SESSION['Status'] == 'SUSER') {
                                ...
                                $sql_dep = mysqli_query($db, "select dep_name as name from department_group where main_dep='$mdepno'");
                            }
                        } else {
                            $sql_dep = mysqli_query($db, "select depName as name from department where depId='$depno'");
                        }
    ...
    <?php include_once ('option/funcDateThai.php'); ?>
                    <table align="center" width="100%" border="1">
                        <?php if ($method == 'Lperson_dep') { ?>
                            <tr>
                                <td colspan="5" align="center"><?= isset($depname['name']) ? $depname['name'] : '' ?></td>
                            </tr>
    <?php } ?>
