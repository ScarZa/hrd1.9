 ...
 <div class="row">
          <div class="col-lg-12">
			...
			<?php 
			 ...
			 $sqlGet=mysqli_query($db,"select * from  department d1
    ...
    <div class="col-lg-4">       
		<form name='form1' class="navbar-form navbar-left"  action='prcdepartment.php' method='post' enctype="multipart/form-data" OnSubmit="return fncSubmit();">
			...
			<label>ฝ่ายงาน </label>
                        <input type='text' name='name'  id='name_dep' placeholder='ฝ่ายงาน' class='form-control'  value='<?= isset($resultGet['dep_name'])?$resultGet['dep_name']:''?>'onkeydown="return nextbox(event, 'save');" required>
			 </div> 
    ...
    <div class="col-lg-12">
		...
		<form name='form2' class="navbar-form navbar-left"  action='prcdepartment.php' method='post' enctype="multipart/form-data" OnSubmit="return fncSubmit();">
			...
			<label>เลือกฝ่ายงาน </label>
                        <select name="md_name" id="md_name" required  class="form-control select2" style="width: 100%" onkeydown="return nextbox(event, 'fname');"> 
				<?php	$sql = mysqli_query($db,"SELECT * FROM department_group order by main_dep ");
          ...
          if($result['main_dep']==$resultGet['main_dep']){$selected='selected';}else{$selected='';}
				 echo "<option value='".$result['main_dep']."' $selected>".$result['dep_name'] ."</option>";
                    ...
                    <div class="form-group">	
			<label> งาน </label>
                        <input type='text' name='name'  id='name_dep' placeholder='งาน' class='form-control'  value='<?= isset($resultGet['depName'])?$resultGet['depName']:''?>'onkeydown="return nextbox(event, 'save');" required>
			 </div> 
