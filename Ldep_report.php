...
if(!empty($take_date1)){  
	...
	echo "<p>&nbsp;</p></center>";
$sql=mysqli_query($db,"select count(mngrisk_id) as number_risk,d1.name as dep_name from mngrisk m1
...
}else if(empty ($take_date1) and empty ($mng_status)){  
	echo "<p>&nbsp;</p><center>";
	echo "แสดงทั้งหมด<BR>";	
	echo "<p>&nbsp;</p></center>";

			
 				$sql = mysqli_query($db,"SELECT COUNT(w.workid) AS count_leave,d.depName as dep_name FROM department d
...
if(!empty($take_date1)){  
$sql=mysqli_query($db,"select count(mngrisk_id) as number_risk,d1.dep_id,d1.name as dep_name from mngrisk m1
				 LEFT OUTER JOIN takerisk t1 on t1.takerisk_id = m1.takerisk_id
				 LEFT OUTER JOIN department d1 on t1.take_dep = d1.dep_id
				 where    t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by d1.dep_id order by number_risk DESC");	
}else  if(empty ($take_date1)){ 			
 				$sql = mysqli_query($db,"SELECT d.depId as depid,  d.depName as name,
