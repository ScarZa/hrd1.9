...
$sql = mysqli_query($db,"select count(re.recomm_id) AS countrecomm from re_comment re
        ...
        WHERE re.check_recomm='N' and re.empno_re!='".$user."' and comm.empno_comm='".$user."'");
...
$sql3 = mysqli_query($db,"select count(comm.comm_id) AS countcomm from comment comm
        ...
        WHERE comm.check_comm='N' and comm.empno_comm!='".$user."' and top.empno_post='".$user."'");
...
WHERE ISNULL(w.enpid) AND ISNULL(p.empno) and f.empno='".$user."' and f.see='N'");
...
WHERE l.empno='".$user."' and l.see='N'");
                                    $result5 = mysqli_fetch_assoc($sql5);
                                    echo mysqli_error($db);
