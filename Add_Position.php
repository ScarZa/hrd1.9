 ...
 <div class="row">
          <div class="col-lg-12">
			...
			<?php 
			 ...
			 $dep_id=$_GET['dep_id'];
			 $sqlGet=mysqli_query($db,"select * from  posid
                             where posId='$dep_id' ");
    ...
    <div class="col-lg-4">       
		<form name='form1' class="navbar-form navbar-left"  action='prcposition.php' method='post' enctype="multipart/form-data" OnSubmit="return fncSubmit();">
			...
			<label>ตำแหน่ง </label>
                        <input type='text' name='name' size="100" id='name' placeholder='ตำแหน่ง' class='form-control'  value='<?= isset($resultGet['posname'])?$resultGet['posname']:''?>'onkeydown="return nextbox(event, 'save');" required>
			 </div> 
...
<?PHP 
 ...
 ?> <br><br>
        <p><button  class="btn btn-success" id='save'> บันทึก </button > <input type='reset' class="btn btn-danger"   > </p>
          ...
          <div class="col-lg-12">
          <?PHP include_once 'listposition.php';?> 
        </div>
	   </div>
