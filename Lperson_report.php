...
<form method="post" action="session.php" enctype="multipart/form-data" class="navbar-form navbar-right">
                  ...
                  <?php if($_SESSION['Status']=='ADMIN' or $_SESSION['Status']=='USUSER'){?>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td>
                                <div class="form-group">
                                    <select name="dep" id="dep"  class="form-control select2" style="width: 200px" onkeydown="return nextbox(event, 'line');"> 
				<?php
                                ...
                                $sql = mysqli_query($db,"SELECT *  FROM department $code order by depId");
 ...
 <?php         
   		$take_date1= isset($_SESSION['check_date01'])?$_SESSION['check_date01']:'';
                $take_date2= isset($_SESSION['check_date02'])?$_SESSION['check_date02']:'';
                include_once ('option/funcDateThai.php');
		$take_rec_date= isset($result['take_rec_date'])?$result['take_rec_date']:'';
		...
		DateThai2($take_date2); //-----แปลงวันที่เป็นภาษาไทย
               ...
               if(isset($_SESSION['dep_Lperson'])?$_SESSION['dep_Lperson']:''=='Lperson_dep'){ 
               $depno= isset($_REQUEST['depname'])?$_REQUEST['depname']:'';}
                }elseif ($_SESSION['Status']=='SUSER') {
                    $depno=$_SESSION['dep'];
                }
                if(!empty($take_date1) and !empty($depno)){  
$sql=mysqli_query($db,"SELECT CONCAT(e1.firstname,' ',e1.lastname) as fullname,e1.empno as empno,
...
<table  id="datatable" align="center" width="100%" border="1">
    <?php if (isset($_SESSION['check_Lperson'])?$_SESSION['check_Lperson']:''=='Lperson_date') { ?>
                        <tr>
                            <td colspan="18" align="center">ตั้งแต่วันที่ <?= DateThai1($take_date1); ?> ถึง <?= DateThai2($take_date2); ?></td>
                        </tr>
<?php } ?>
                        <tr>
                            <td colspan="18" align="center"><?= isset($depname['name'])?$depname['name']:''?></td>
                        </tr>
