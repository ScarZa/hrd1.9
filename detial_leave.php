...
if (!empty($_REQUEST['work_id'])) {
    ...
    $del_photo=mysqli_query($db,"select pics from work where workid ='$work_id'");
    ...
    $del_photo=mysqli_query($db,"select pics_t from timela where id='$time_id'");
                $del_photo=mysqli_fetch_assoc($del_photo);
...
if (!empty($_POST['check_date01'])) {
    ...
    $detialscan = mysqli_query($db,"SELECT f.* FROM fingerprint f 
...
WHERE ISNULL(w.enpid) AND ISNULL(p.empno) and f.empno='$empno' and (f.forget_date between '$date01' and '$date02') order by f.finger_id desc");
    ...
    $detiatl = mysqli_query($db,"SELECT * from timela where empno='$empno' and status='N'  and datela BETWEEN '$Y-10-01' and '$y-09-30' order by id desc");
    $detialscan = mysqli_query($db,"SELECT f.* FROM fingerprint f 
LEFT JOIN `work` w ON f.empno=w.enpid AND f.forget_date BETWEEN w.begindate AND w.enddate AND w.statusla='Y'
LEFT JOIN plan_out p ON f.empno=p.empno AND f.forget_date BETWEEN p.begin_date AND p.end_date
WHERE ISNULL(w.enpid) AND ISNULL(p.empno) and f.empno='$empno' and (f.forget_date between '$Y-10-01' and '$y-09-30') order by f.finger_id desc");
    ...
    $detiatl = mysqli_query($db,"SELECT * from timela where empno='$empno' and status='N'  and datela BETWEEN '$y-10-01' and '$Yy-09-30' order by id desc");
    $detialscan = mysqli_query($db,"SELECT f.* FROM fingerprint f 
LEFT JOIN `work` w ON f.empno=w.enpid AND f.forget_date BETWEEN w.begindate AND w.enddate AND w.statusla='Y'
LEFT JOIN plan_out p ON f.empno=p.empno AND f.forget_date BETWEEN p.begin_date AND p.end_date
...
<?php } ?>
                                        <tr align="center" bgcolor="#898888">
                                            ...
                                            <tr>
                                                ...
                                                <td align="center"><?php if (!empty($result['pics'])) {
...
<?php
if($date >= $bdate and $date <= $edate){
$count_check = mysqli_query($db,"select sum(total) as sum from timela where empno='$empno'  and datela BETWEEN '$y-10-01' and '$Yy-09-30' and status='N' ");
}  else {
$count_check = mysqli_query($db,"select sum(total) as sum from timela where empno='$empno'and datela BETWEEN '$Y-10-01' and '$y-09-30'  and status='N' ");    
}
...
<?php } ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td align="center"><?php if (!empty($result['pics_t'])) {
                                ...
                                <div class="panel-heading">
                                 ...
                                 <div class="panel-body">
                                        <table align="center" width="100%" border="0" cellspacing="0" cellpadding="0" class="divider" rules="rows" frame="below">
                                            ...
                                            <?php $i = 1;
                                                        while ($result = mysqli_fetch_assoc($detialscan)) {?>
                                                <tr>
                                                    <td align="center"><?= $i?></td>
                                 ...
                                 <div class="panel-body">
                                        <table align="center" width="100%" border="0" cellspacing="0" cellpadding="0" class="divider" rules="rows" frame="below">
                                            <tr align="center" bgcolor="#898888">
                                             ...
                                             <?php $i = 1;
                                                        while ($result = mysqli_fetch_assoc($detiallate)) {?>
                                                <tr>
                                                    <td align="center"><?= $i?></td>
